...
![通知間隔を変えるとDB負荷と状態発見の遅れが同時に変わる因果関係](./images/10-notification-interval.svg)

_間隔を長くすると空問い合わせは減りますが、状態発見が遅れます。局所的なDB負荷だけでなく、ride完了数と総スコアまで確認して採否を決めます。_

> **用語補足**
>
> - **polling**: 一定間隔で状態を問い合わせる方式です。
> - **micro benchmark**: 1関数や1queryだけを切り出す計測です。利用者行動まで含む総合ベンチとは結果が一致しない場合があります。
> - **cache**: 前回の認証結果や通知内容を再利用し、同じ計算やSQLを省く仕組みです。元データの更新時には古いcacheを無効化する必要があります。
> - **long polling / SSE**: clientが短い間隔で問い合わせ続けず、server側で変化を待つ、または接続中に変化をpushする方式です。
