...
![座標更新で時刻の再SELECTをなくし、rideとstatusを1 SQLへまとめる処理フロー](./images/08-coordinate-hot-path.svg)

_変更後は同じ時刻をINSERTとresponseへ使い、current rideとlatest statusをまとめて取得します。通常経路は4回から2回のDB往復になります。_

> **用語補足**
>
> - **hot path**: request中に高頻度で通るため、全体負荷へ大きく効く処理経路です。
> - **DB往復**: アプリがqueryを送り、DBが実行して結果を返すまでの1往復です。
> - **read-after-write**: 書いた値を確認・取得するため、直後に読み直す処理です。
> - **相関subquery**: 外側queryの値（ここではride ID）を条件に使う内側queryです。
