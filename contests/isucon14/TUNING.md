...
drive区間を同一ride IDで追跡した結果、抽出74 rideでdrive tickを増やし得る
1,191 coordinate POST中1,031件がclientから見て30ms以上でした。server平均76.515msのうち
...
Benchmark 52では、`GET /api/app/rides` のride別status / coupon / chair / owner N+1を
1 statementへまとめました。通常3走は124,205–133,737点、推定代表値の中央値
128,584点で、直前の同期実装中央値127,499点から+0.85%です。全走
`pass=true`・error map空でした。点差はrun間の幅より小さい一方、変更前に
...
`eval reqs` とmatching / pickup / pickup + drive合算不満率を読みます。
...
| [40-drive-phase-diagnostics.md](./tuning/40-drive-phase-diagnostics.md) | benchmarkerの理想 / 実tickと、1/32 rideのcoordinate、CARRYING commit、app / chair poll response構築を同じride IDで相関 | 全2,310 rideのdrive不満77.3%。抽出74 ride・1,191 POSTの86.6%がclient 30ms以上。server平均76.515msの84.1%がpool待ち。余分3,718 tick対blocked見積り3,678 tick | 診断instrumentation付き実測n=1・未推定。score 146,727、`pass=true`・error map空。Rust / Go barrierと`dropped_lines=0`を確認し、tick / commit / 周期filter fixtureも追加。current UPDATEよりpool待ちが支配的なため、総接続50内の用途別予約を次に比較 |
| [41-db-pool-partition-16.md](./tuning/41-db-pool-partition-16.md) | 総接続50をgeneral 34 / coordinate 16へ分割 | 128,038点、完了1,979、drive不満79.5%、周期sampleのcoordinate pool平均71.623ms | 診断n=1・未推定。`pass=true`・error map空。generalは改善したがcoordinate 16が不足して不採用 |
| [42-db-pool-partition-24.md](./tuning/42-db-pool-partition-24.md) | general 26 / coordinate 24へ再配分 | 152,128点、完了2,386、drive不満74.0%、周期sampleのcoordinate pool平均30.414ms | 診断n=1・未推定。`pass=true`・error map空。coordinateは改善したがgeneral飽和のため通常runへ進めた |
| [43-db-pool-partition-20.md](./tuning/43-db-pool-partition-20.md) | general 30 / coordinate 20の中間比較 | 145,732点、完了2,327、drive不満80.6%、周期sampleのcoordinate pool平均62.854ms | 診断n=1・未推定。`pass=true`・error map空。generalは中間だがcoordinate待ちが共有時相当 |
| [44-db-pool-partition-adoption.md](./tuning/44-db-pool-partition-adoption.md) | general 26 / coordinate 24を通常runで採用判定 | 3走133,797–142,851点、中央値138,027点、直前比+3.6%。既定値確認132,756点 | 比較3走は全て`pass=true`・error map空。実効設定4走の記述中央値135,912点、直前比+2.0% |
