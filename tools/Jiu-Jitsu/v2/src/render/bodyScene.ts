...
export class BodyScene {
  ...
  show(pair: PracticePair): void {
    this.pair = pair;
    for (const body of [this.blue, this.red]) if (body) { this.scene.remove(body.root); body.dispose(); }
    this.blue = new Mannequin(pair.blue, 0x629de6);
    this.red = new Mannequin(pair.red, 0xd87966);
