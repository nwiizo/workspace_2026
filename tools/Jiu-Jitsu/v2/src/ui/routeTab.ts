...
import { h } from "./dom";
import { loadRoute, saveRoute } from "../engine/routeStorage";
...
export class RouteTab {
  ...
  constructor(private readonly store: KeyValueStore, private readonly page: "edit" | "watch", private readonly openOther: (item: SuggestedRoute) => void) {
    ...
    try {
      ...
      this.saveStatus.textContent = saved ? "このブラウザに保存したルートを読み込みました。" : "姿勢を登録すると、このブラウザに作成中の1ルートを保存します。";
    ...
    const canvas = h("canvas", { attrs: { "aria-label": "点をドラッグして人形を動かす。部位と座標の入力でも操作できます。" } });
    ...
    this.root = h("div", { class: `route-editor route-${page}` },
      h("div", { class: "route-heading" }, h("h2", { text: page === "edit" ? "人形で、動きのルートを作る" : "ルートを確認する" }),
        ...
        h("button", { class: "btn btn-start", text: page === "edit" ? "このルートを確認する" : "保存した自作ルートを確認", onClick: () => {
          if (page === "edit") {
    ...
    this.scene = new BodyScene(canvas, pins); this.selectFrame(0);
  ...
  private customExample(): SuggestedRoute {
    const frames = this.frames.map((f) => ({ ...f, pose: cloneRoutePose(f.pose) }));
    return { id: "custom", name: this.name.value, fact: "ルート作成で登録した姿勢を順番に再生します。", exercise: "途中に指摘があれば、作成ページで中間姿勢を追加して調整できます。", frames: () => frames.map((f) => ({ ...f, pose: cloneRoutePose(f.pose) })) };
  }
  ...
  editExample(item: SuggestedRoute): void {
    ...
    if ((this.dirty || this.saved || this.unsaved) && !window.confirm("作成中のルートをこの登録例で置き換えますか？ 保存済みの自作ルートも置き換わります。")) return;
