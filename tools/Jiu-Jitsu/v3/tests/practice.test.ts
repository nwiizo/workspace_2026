...
describe("一手ずつのポジション練習", () => {
  it("ハーフガードの膝盾を潰されたときは、脚を戻す前にフレームを作る", () => {
    const run = new PracticeRun("half-bottom", "check", 1);
    move(run, "recover");
    expect(run.nodeId).toBe("flat");
    move(run, "frame"); move(run, "knee"); move(run, "recover");
    expect(run.phase).toBe("complete");
    expect(practicePair(run.lesson.id, run.nodeId).blue.chest.y).toBeLessThan(practicePair(run.lesson.id, run.nodeId).red.chest.y);
  });
  it("タートルで首への手が来たら、回るより先に手を戻してからガードへ進む", () => {
    const run = new PracticeRun("turtle-bottom", "check", 1);
    move(run, "turn");
    expect(run.nodeId).toBe("reach");
    move(run, "protect"); move(run, "read"); move(run, "turn");
    expect(run.phase).toBe("complete");
    expect(run.node.title).toContain("ガード");
  });
