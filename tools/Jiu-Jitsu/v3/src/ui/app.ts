...
import { DuelTab as DojoTab } from "./duelTab";
  ...
  constructor(root: HTMLElement, store: KeyValueStore) {
    ...
    const header = h("header", { class: "app-header" },
      h("div", { class: "app-brand" },
        h("h1", { class: "app-title", text: "柔術道場 3" }),
        h("span", { class: "app-subtitle", text: "v3.0.0 / 動きと攻防をつなぐ" }),
      ),
      nav,
      this.beltEl,
    );
    ...
    const followAddress = () => {
      ...
      this.switchTab(TABS.some(([tab]) => tab === id) ? id : "lab");
    };
    window.addEventListener("hashchange", followAddress);
    if (window.location.hash) followAddress();
  }
