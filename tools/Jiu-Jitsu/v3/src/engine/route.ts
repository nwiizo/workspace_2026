...
import { HIP_RADII } from "../render/mannequinShape";
...
export function inspectRoutePose(pose: RoutePose): { pair: PracticePair; issues: MotionIssue[] } {
  ...
  for (const actor of ["blue", "red"] as const) {
    ...
    for (const [side, label] of [["L", "左"], ["R", "右"]] as const) {
      const hip = body[`hip${side}`];
      floor(ovalMinimum(hip, new Vector3(...HIP_RADII), body.rotation), `${label}の股関節まわり`, hip);
    }
    ...
    for (const side of ["L", "R"] as const) {
      ...
      for (const [kind, root, middle, end, a, b] of [
        ...
        floor(target.end.y - (kind === "legs" ? 0.055 : 0.046), `${sideName}${kind === "legs" ? "足" : "手"}の目標`, target.end);
