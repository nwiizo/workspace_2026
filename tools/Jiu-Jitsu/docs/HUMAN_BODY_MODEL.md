...
現行版は `v3/`。固定長IKを継続し、姿勢更新時は同じメッシュの位置・向き・スケールを更新する。手首だけでなく手の厚みも接地検査へ含めた。ハーフガードとタートルは専用の二人の配置を追加し、全練習の支持点・骨長・床・身体内部への侵入を検証する。

ルート内の小さな動きは補間して途中も検査する。未実装の返し・パスなどは省略を明示した局面切り替え。対戦は `duel.ts` の状態に応じて `duelPose.ts` が配置を選び、重力・摩擦・接触力で自由に組み合う計算はしない。詳細は [v3の技術選定](../v3/docs/TECH_SELECTION.md) と [扱う範囲](../v3/README.md#身体モデルが扱う範囲)。以下はv2以前の背景。

## v2 ポジション練習・応用ロールの現在の方式

ポジション練習は `v2/src/render/practicePose.ts`、応用ロールは `rollPose.ts` で二人の配置を作り、`mannequin.ts` / `bodyScene.ts` で共通の人形を描く。膝・足の接地と二人の位置関係を先に定め、固定長の腕・脚を手足の目標へ合わせる。膝の球の半径を含めて床へ置き、曲げる側を指定して中間の関節を計算する。胸郭・骨盤と四肢の厚み、骨格の表示は同じ関節位置を使う。

各状態を静止した姿勢として描画し、一手の前後を同じ視点で比較する。姿勢の単純な補間で床や相手を通り抜ける途中の動作は表示しない。身体と骨格の切り替え、相手の透過、支持点の丸印を使い、説明文と身体の配置が一致しているかを検証する。

全状態で支持点・骨長・描画メッシュと床・相手の胸郭／骨盤内部への四肢の侵入をテストする。内部への侵入検査は代表点による近似であり、表面同士の完全な衝突判定ではない。検証画面は `v2/_practice-audit.html`。骨格は教育用の模式図で、個々の骨の形、筋活動、軟部組織の変形、接触力、摩擦は再現しない。

以下は従来の v1 および v2 関節資料の角度リグの方針。練習・応用ロールの人形に、単体の root 全体を動かす接地補正を重ねないこと。
