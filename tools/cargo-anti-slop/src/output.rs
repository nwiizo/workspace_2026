...
pub fn write_text<W: Write>(
    ...
    writeln!(writer, "Anti-Slop: {}", analysis.project)?;
    if options.japanese {
        writeln!(
            writer,
            "評価: {} | 解析ファイル: {} | issue: {}",
    ...
    if issues.is_empty() {
        ...
        for issue in issues {
            ...
            if options.print
                && let Some(line) = source_line(&mut sources, &analysis.root, issue)
            {
                writeln!(writer, "  > {line}")?;
            }
            writeln!(
...
pub fn write_ai<W: Write>(
    ...
    if options.japanese {
        writeln!(writer, "# cargo-anti-slop 修正指示")?;
        writeln!(writer, "評価: {}", analysis.grade)?;
        writeln!(writer, "優先順に以下を修正してください。")?;
    } else {
        writeln!(writer, "# cargo-anti-slop repair plan")?;
    ...
    for issue in filtered_issues(analysis, options.all) {
        if options.japanese {
            ...
            writeln!(
                writer,
                "- 検証: `cargo anti-slop --check` と関連する Rust テストを再実行"
            )?;
        } else {
            writeln!(
                writer,
            ...
            writeln!(
                writer,
                "- validation: rerun `cargo anti-slop --check` and relevant Rust tests"
            )?;
        }
    }
    if let Some(diff) = baseline {
...
fn source_line(
    ...
    let lines = cache
        .entry(issue.rel_path.clone())
        .or_insert_with(|| {
            std::fs::read_to_string(root.join(&issue.rel_path))
                .ok()
