...
use cargo_anti_slop::analyze_path;
use cargo_anti_slop::baseline::diff_against_ref;
use cargo_anti_slop::config::Config;
use cargo_anti_slop::error::Result;
use cargo_anti_slop::issue::Severity;
use cargo_anti_slop::output::{OutputOptions, write_ai, write_blind_spots, write_json, write_text};
use clap::{Parser, ValueEnum};

#[derive(Debug, Parser)]
#[command(name = "cargo-anti-slop")]
#[command(
    author,
    version,
    about = "Diagnose low-evidence (anti-slop) patterns in Rust code"
)]
struct Args {
    ...
    /// Include the offending source line in human output.
    #[arg(long)]
    print: bool,
...
{
    design_gate_core::absorb_cargo_subcommand(args, "anti-slop")
}
...
fn gate_report(
    issues: &[cargo_anti_slop::Issue],
    baseline: Option<&cargo_anti_slop::baseline::BaselineDiff>,
    fail_on: Severity,
) -> cargo_anti_slop::output::GateReport {
    let issues = baseline
