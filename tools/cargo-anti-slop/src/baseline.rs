...
pub fn diff_against_ref(
    ...
    let worktree =
        design_gate_core::prepare_baseline_worktree(current_path, git_ref, "cargo-anti-slop")?;
...
mod tests {
        ...
        Analysis {
            ...
            issues: vec![Issue {
                key: IssueKey {
                    ...
                    target: "return:serde_json::Value".to_string(),
                },
    ...
    #[test]
    fn renamed_source_is_new_and_resolved() {
        let diff = diff(&analysis("src/lib.rs:fetch"), &analysis("src/lib.rs:load"));
