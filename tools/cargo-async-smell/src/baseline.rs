...
use crate::analyzer::{Analysis, Runtime, analyze_path};
...
pub fn diff_against_ref(
    ...
    let worktree =
        design_gate_core::prepare_baseline_worktree(current_path, git_ref, "cargo-async-smell")?;
    let baseline = analyze_path(worktree.baseline_path(), config, runtime)
...
mod tests {
    ...
    use crate::issue::{IssueKey, IssueType, RiskAxis, Severity};
    ...
    fn analysis(source: &str) -> Analysis {
        Analysis {
            ...
            root: PathBuf::from("."),
            runtime: crate::analyzer::Runtime::Tokio,
            files_analyzed: 1,
            suppressed_issues: 0,
    ...
    fn identical_keys_are_unchanged() {
        let diff = diff(&analysis("src/lib.rs:fetch"), &analysis("src/lib.rs:fetch"));
