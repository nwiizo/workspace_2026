//! Convolution (NTT: Number Theoretic Transform)
//!
...
///
/// Computes a(x) * b(x) mod 998244353
///
/// # Complexity
/// O((N + M) log (N + M))
///
/// # Example
/// ```
/// use procon_lib::convolution::convolution;
///
...
pub fn convolution(a: &[i64], b: &[i64]) -> Vec<i64> {
    ...
    let mut fa: Vec<i64> = a.iter().map(|&x| x.rem_euclid(MOD)).collect();
    let mut fb: Vec<i64> = b.iter().map(|&x| x.rem_euclid(MOD)).collect();
    fa.resize(n, 0);
    fb.resize(n, 0);
...
/// Convolution with arbitrary modulo using Garner's algorithm
///
/// # Example
/// ```
/// use procon_lib::convolution::convolution_mod;
///
/// let a = vec![1, 2];
/// let b = vec![3, 4];
...
/// Convolution returning i64 without modulo (for small inputs)
///
/// Uses convolution_mod with a large modulo to recover exact values.
pub fn convolution_i64(a: &[i64], b: &[i64]) -> Vec<i64> {
    // Use three NTT primes and CRT
    convolution_mod(a, b, i64::MAX)
}

/// Self-convolution (squaring a polynomial)
pub fn convolution_self(a: &[i64]) -> Vec<i64> {
    convolution(a, a)
}
...
mod tests {
    ...
    #[test]
    fn test_convolution_self() {
        let a = vec![1, 2, 3];
        let c = convolution_self(&a);
        assert_eq!(c, vec![1, 4, 10, 12, 9]);
    }
}
