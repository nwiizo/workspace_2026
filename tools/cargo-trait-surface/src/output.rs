...
use crate::analyzer::{Analysis, TraitDetail};
...
pub fn write_text<W: Write>(
    ...
    writeln!(writer, "Trait Surface: {}", analysis.project)?;
    if options.japanese {
        writeln!(
            writer,
            "評価: {} | 解析ファイル: {} | issue: {}",
...
pub fn write_ai<W: Write>(
    ...
    if options.japanese {
        writeln!(writer, "# cargo-trait-surface 修正指示")?;
        writeln!(writer, "評価: {}", analysis.grade)?;
        writeln!(writer, "優先順に trait 境界を見直してください。")?;
    } else {
        writeln!(writer, "# cargo-trait-surface repair plan")?;
        writeln!(writer, "Grade: {}", analysis.grade)?;
        writeln!(
            writer,
            "Review these trait boundary findings in priority order."
        )?;
    }
    write_gate(options, writer)?;
    for issue in filtered_issues(analysis, options.all) {
        if options.japanese {
            ...
            writeln!(
                writer,
                "- 検証: `cargo trait-surface --check` と関連テストを再実行"
            )?;
        } else {
            writeln!(
                writer,
            ...
            writeln!(
                writer,
                "- validation: rerun `cargo trait-surface --check` and relevant Rust tests"
            )?;
        }
    }
    if let Some(diff) = baseline {
...
pub fn write_trait_detail<W: Write>(
    detail: Option<&TraitDetail>,
    japanese: bool,
    writer: &mut W,
) -> io::Result<()> {
    let Some(detail) = detail else {
        return writeln!(
            writer,
            "{}",
            localized(japanese, "trait が見つかりません。", "trait not found.")
        );
    };
    if japanese {
        writeln!(writer, "トレイト: {}", detail.name)?;
        writeln!(
            writer,
            "場所: {}:{} | public: {} | object-safe: {}",
            detail.file.display(),
            detail.line,
            detail.public,
            detail.object_safe
        ...
        writeln!(
            writer,
            "メソッド: {} | 関連型: {}",
            detail.method_count, detail.associated_type_count
        )?;
    } else {
        writeln!(writer, "Trait: {}", detail.name)?;
        writeln!(
            writer,
            "Location: {}:{} | public: {} | object-safe: {}",
            detail.file.display(),
            detail.line,
            detail.public,
            detail.object_safe
    ...
    write_list(
        writer,
        localized(japanese, "メソッド一覧", "Methods"),
        &detail.methods,
    )?;
    write_list(
        writer,
        localized(japanese, "impl 一覧", "Impls"),
        &detail.impls,
    )?;
    write_list(
        writer,
        localized(japanese, "dyn 利用箇所", "dyn uses"),
        &detail.dyn_uses,
    )
}

fn write_list<W: Write>(writer: &mut W, title: &str, items: &[String]) -> io::Result<()> {
...
fn localized_count(japanese: bool, count: usize, ja: &str, en: &str) -> String {
    if japanese {
        format!("{count} {ja}")
    } else {
        format!("{count} {en}")
