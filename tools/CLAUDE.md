...
| `nwiizo-claude-plugins/` | 個人用 Claude Code プラグインマーケットプレイス。`~/.claude` の `home-*` skills/agents を配布可能な 6 プラグインに再編成（nwiizo-review / engineering-discipline / finops-investigation / prompt-engineering / dev-workflow / authoring）。`home-` プレフィックス除去でプラグイン名 namespace 化、`.claude-plugin/marketplace.json` 準拠。17 skills + 5 agents 同梱。README に `anthropics/claude-plugins-official` の精選有効化リスト・重複分析・再現手順も統合 |
...
| `cargo-anti-slop/` | 低根拠 (low-evidence) パターン診断 cargo サブコマンド (Rust, ra_ap_syntax + design-gate-core)。dmmulroy/anti-slop (Oxlint) の Rust 再解釈 + 「型は壁」講演の関数境界側。12 ルール: chained-cast (narrow→widen マスク)・erased-signature/alias/dictionary (Value / dyn Any / 消去 value map)・runtime-downcast (error 風 receiver は低スコア)・widen-then-assert (境界パース from_str 系は除外)・default-swallow (`.ok();` / fallible 後 unwrap_or_default)・vague-symbol-name (語彙 config 可)・bool-validation (証拠を捨てる bool 検証)・raw-id-params (同生型 `*_id` 引数の取り違え)・constructor-bypass (smart constructor を derive(Default/Deserialize) / From が迂回)・orphan-file (module tree 未到達のリファクタ残骸、内容 finding はマスク)。`#[serde(flatten)]` は宣言済み境界として Low、shallow clone では volatility 軸を自動無効化、Grade は Medium+ 主導 (Low は A→B 降格のみ)。`--print` で該当行表示。trait impl の signature は skip、`// anti-slop-allow: rule -- reason` suppress。struct フィールドの壁 (bool-option-pair / raw-id-field 等) と unwrap/panic 系は rbp-lint 管轄で非重複 |
...
| `shuhari/` | 競合の一次情報を調査し、対象リポジトリに適した最小の機能実装と検証へつなげるAgent Skill |
| `kindle-app-rs/` | Kindle Web ReaderまたはmacOSアプリをキャプチャし、PDF生成・トリミング・復元可能な重複整理を行うRust CLI |
