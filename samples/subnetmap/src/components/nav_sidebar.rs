...
pub fn NavSidebar() -> impl IntoView {
    view! {
        <nav class="fixed left-0 top-0 h-full w-64 bg-slate-900 border-r border-slate-700 flex flex-col">
            <div class="p-5 border-b border-slate-700">
                <a href="/" class="flex items-center gap-3">
                    <div class="w-8 h-8 bg-blue-500 rounded-lg flex items-center justify-center font-bold text-sm">
                        "SM"
                    </div>
                    <span class="text-lg font-semibold text-white">"SubnetMap"</span>
                </a>
            </div>

            <div class="flex-1 py-4 space-y-1 px-3">
                <NavItem href="/" label="Dashboard" icon="grid" />
                <NavItem href="/subnets" label="Subnets" icon="network" />
                <NavItem href="/ips" label="IP Addresses" icon="server" />
                <NavItem href="/vlans" label="VLANs" icon="layers" />
                <NavItem href="/audit" label="Audit Log" icon="history" />
